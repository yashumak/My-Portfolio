...
const Tech = () => {
  const renderTechButtons = (techArray) => {
    return techArray.map((technology) => (
      <motion.div
        key={technology.id || technology.name || technology.icon}
        variants={fadeIn("top", "spring", 0.25, 1.25)}
      >
        <Button
          disabled
          variant="outlined"
      ...
      <div className="mt-10">
        <div className="flex flex-row flex-wrap justify-center gap-2">
          <Accordion defaultExpanded sx={accordionStyle}>
            <AccordionSummary
              expandIcon={<ExpandMoreIcon sx={accordionStyle} />}
          ...
          {/* Added Other Technologies Accordion */}
          <Accordion sx={accordionStyle}>
            <AccordionSummary
              expandIcon={<ExpandMoreIcon sx={accordionStyle} />}
